...
def search_for_keyword(top_3_words):
	print("Search Keywords for Response!")
	print("----------------------------------")
	...
	print("RESPONSE PRE: ",response_predecessor)
	print("RESPONSE SUCC: ",response_successor)
...
# schleife solange bis abbruchsymbol
	...
	def search_predecessor_chain(satz):
		...
		if predecessor_anfrage_len > 0:
			...
			print(response_predecessor,ergebnis)
			if "!$!" in ergebnis:
				print("Vorheriger Satz erkannt! Beginne hier:")
	...
	def search_successor_chain(satz):
		...
		successor_anfrage = c.execute('SELECT kette, probability FROM Kette WHERE\
		(kette like ?) AND NOT (kette like ?) ORDER BY probability DESC limit 1 ',[neuerSatz+"%","%"+satz+"%"])
		successor_anfrage_len = len(successor_anfrage.fetchall())
		print("Successor_len: ",successor_anfrage_len)
		
		''' to do: vernueftige fehlerbehandlung! wenn kein ergebnis mehr vorliegt was dannmachen?'''
		for row in c.execute('SELECT kette, probability FROM Kette WHERE \
		(kette like ?) AND NOT (kette like ?) ORDER BY probability DESC limit 1 ',[neuerSatz+"%","%"+satz+"%"]):
			ergebnis = row
		if successor_anfrage_len > 0:
			ergebnis = list(ergebnis)
			ergebnis = ergebnis[0]
			print("ERGEBNIS SUCC", ergebnis)
			row_ergebnis = ergebnis
			conn.commit()
			conn.close()
			...
			#if row_ergebnis == response_predecessor:
			#	print("ja")
...
if __name__ == "__main__":
	...
	#top_3_words = get_top_entropies("i want an quite intense one")
